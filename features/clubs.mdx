...
<Warning>
  In `clubs.hbs`, only the **TechnoX** card links to the correct Express route
  (`/techno`). The **Bizeco**, **NSS Club**, and **Niyantran** cards all use
  `href="techno.html"` — a bare HTML file path that does not correspond to any
  route registered in `src/index.js`. Clicking those three cards will produce a
  404 in the Express app. Each club needs its own route and template (e.g.
  `GET /bizeco`, `GET /nss`, `GET /niyantran`) or the hrefs should be updated to
  point to a shared club detail route.
</Warning>
